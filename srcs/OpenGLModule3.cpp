...
OpenGLModule3::OpenGLModule3(Nibbler & nibbler, Board & board) :
...
void			OpenGLModule3::_initOpenGLStuff(void)
{
...
void			OpenGLModule3::_initModels(void)
{
	glm::mat4	tempMatrix;
...
void				OpenGLModule3::_initLightPosition(void)
{
...
void			OpenGLModule3::_resetGraphicsParameters(void)
{
...
OpenGLModule3::~OpenGLModule3(void)
{
	printf("~OpenGLModule3()\n");
...
void			OpenGLModule3::disable(void)
{
	// printf("OpenGLModule3: disable()\n");
	SDL_HideWindow(this->_window);
}

void			OpenGLModule3::enable(void)
{
	// printf("OpenGLModule3: enable()\n");
	SDL_GL_MakeCurrent(this->_window, this->_context);
	SDL_ShowWindow(this->_window);
}

void			OpenGLModule3::handleEvents(void)
{
	SDL_Event	event;
...
void			OpenGLModule3::_handleKeyHeldEvent(void)
{
...
void			OpenGLModule3::_handleKeyPressEvent(SDL_Event & event)
{
	switch (event.key.keysym.sym)
	{
		...
		case SDLK_TAB:
			printf("OpenGLModule3: Pressed TAB\n");
			this->_nibbler.selectNextModule();
			return;

		case SDLK_d:
			printf("OpenGLModule3 Module: Pressed D\n");
			break;
...
void			OpenGLModule3::_toggleWireframeMode(void)
{
	this->_isWireframe = !this->_isWireframe;
	glPolygonMode(GL_FRONT_AND_BACK, this->_isWireframe ? GL_LINE : GL_FILL);
}

void			OpenGLModule3::_toggleShading(void)
{
	this->_isShaded = !this->_isShaded;
	this->_shader->setBool("is_shaded", this->_isShaded);
}

void			OpenGLModule3::_toggleView(void)
{
	this->_isFirstPersonView = !this->_isFirstPersonView;
}

void			OpenGLModule3::_drawModelAtPosition(Model & model, int x, int y)
{
...
void			OpenGLModule3::_drawModelAtPositionFacing(Model & model, int x, int y, e_direction direction)
{
...
void			OpenGLModule3::render(void)
{
...
void				OpenGLModule3::_updateViewMatrix(void)
{
...
void			OpenGLModule3::_renderFloorAndWalls(void)
{
...
void			OpenGLModule3::_renderCells(void)
{
	Cell *		cell;
...
void			OpenGLModule3::_renderCellAtPosition(Cell & cell, int x, int y)
{
	SnakeCell *			snakeCell;
	FoodCell *			foodCell;
	EnemyCell *			enemyCell;
	...
	if ((snakeCell = dynamic_cast<SnakeCell *>(&cell)))
		this->_renderSnakeCell(*snakeCell, x, y);
	else if ((foodCell = dynamic_cast<FoodCell *>(&cell)))
		this->_renderFoodCell(*foodCell, x, y);
	else if ((enemyCell = dynamic_cast<EnemyCell *>(&cell)))
		this->_renderEnemyCell(*enemyCell, x, y);
	else
		;	// some other type of cell?

}

void			OpenGLModule3::_renderSnakeCell(SnakeCell & snakeCell, int x, int y)
{
	Model *		model;
	...
	this->_drawModelAtPositionFacing(*model, x, y, snakeCell.getDirection());
}

void			OpenGLModule3::_renderFoodCell(FoodCell & foodCell, int x, int y)
{
	(void)foodCell;

	this->_drawModelAtPosition(*this->_foodModel, x, y);
}

void			OpenGLModule3::_renderEnemyCell(EnemyCell & enemyCell, int x, int y)
{
	(void)enemyCell;

	this->_drawModelAtPositionFacing(*this->_enemyModel, x, y, enemyCell.getDirection());
}
